...
В этом режиме SDK не ждёт роли пользователя в JWT. Сначала он читает access
groups из RPT:
...
Если gateway положил в RPT только page permission, SDK сам проверит element
resource через Keycloak UMA decision endpoint:
...
То есть gateway может проверять только `page.calculator`, а SDK отдельно
спросит Keycloak про `calculator.usage` или другую access group элемента.
