...
`pageInstanceId` не является authorization token. Если в `pagesdk.Config`
задан `Authenticator`, Application проверяет Bearer token на каждом render,
event, callback и delete-запросе. Instance сохраняет только стабильный owner
ID, а не JWT. Встроенный Keycloak JWT authenticator использует
`{issuer}|{subject}` (`iss` + `sub`).

При event/delete текущий owner должен совпасть с owner, сохранённым во время
render. При несовпадении возвращается `404`, чтобы не раскрывать существование
чужого instance.

Если `Authenticator` не задан, сохраняется legacy-режим без аутентификации.
Для production приложения с пользовательскими данными этот режим использовать
не следует.
